...
const TARGET = {
  brand: "Volkswagen",
  model: "Golf",
  modelUrl: "https://www.ultimatespecs.com/car-specs/Volkswagen-models/Volkswagen-Golf",
};
...
const outputPath = path.join(
  ...
  "data",
  "import",
  "vehicleCatalog.ultimatespecs.test.json"
);
...
async function scrapeGolfGenerations() {
  const html = await fetchHtml(TARGET.modelUrl);
  const $ = cheerio.load(html);

  const generations = [];

  const wantedGenerations = [
    "Golf 8 Facelift",
    "Golf 8",
    "Golf 7 Facelift",
    "Golf 7",
  ...
  $("h2").each((_, element) => {
    ...
    if (!parsed) return;
    if (!wantedGenerations.includes(parsed.name)) return;
  ...
  $("a").each((_, link) => {
    ...
    if (!text || !href) return;
    if (!text.toLowerCase().includes("versions")) return;
...
async function scrapeGenerationEngines(generation) {
  ...
  for (const generationLink of generation.links) {
    ...
    $("h1, h2, h3, h4, a").each((_, element) => {
      ...
      if (!href.includes("/car-specs/Volkswagen/")) return;
      if (!rawLabel.toLowerCase().startsWith("golf")) return;
      if (rawLabel.toLowerCase().includes("versions")) return;
      if (rawLabel.toLowerCase() === "golf") return;
...
async function main() {
  ...
  const generations = await scrapeGolfGenerations();
  ...
  const selectedGenerations = generations.filter((generation) =>
    ["Golf 7", "Golf 7 Facelift", "Golf 8"].includes(generation.name)
  );
